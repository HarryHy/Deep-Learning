...
class BasicBlock(nn.Module):
	...
	def __init__(self, in_planes,planes,stride=1, downsample=none):
		...
		self.conv1 = conv3_3(in_planes,planes,stride)
...
class PreActBottleneck(nn.Module):
 ...
 class ResNet_Cifar (nn.Module):
 	...
 	def__init__(self, block, layers, num_classes=10):
 		...
 		self.layer2= self._make_layer(black,32,layers[1],stride=2)
 		self.layer3= self._make_layer(black,64,layers[2],stride=2)
 		self.avgpool = nn.AvgPool2d(8, stride =1)
 		self.fc = nn.Linear(64 * block.expansion, num_classes)

 		for m in self.modules():
 			if isinstance(m, nn,Conv2d):
 				n = m.lernel_size[0] * m.kernel_size[1] * m.out_channels
